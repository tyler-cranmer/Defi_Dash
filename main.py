...
'Nexus Mutual (NXM)', 'Perpetual Protocol (PERP)', 'Serum (SRM)', 'Bancor Network Token (BNT)', 'xSUSHI (XSUSHI)', '0x (ZRX)', 'Raydium (RAY)', 'UMA (UMA)', 
'Coin98 (C98)', 'Wonderland (TIME)', 'Liquity USD (LUSD)', '1inch (1INCH)', 'Neutrino USD (USDN)', 'Loopring (LRC', 'Fei Protocol (FEI)', 'Kava (KAVA)', 'Rocket Pool (RPL)',
'Gnosis (GNO)', 'Reserve Rights Token (RSR)', 'Alpha Finance (ALPHA)', 'Frax (FRAX)', 'Convex Finance (CVX)', 'Rari Governance Token (RGT)', 'Keep Network (KEEP)', 'sETH (SETH)']
...
if __name__ == '__main__':
    ...
    if len(coin_name) == len(coin_links):
        for i in range(len(coin_name)):
            create(coin_name[i], coin_links[i])
            time.sleep(1)